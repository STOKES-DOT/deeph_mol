...
class GATlayer(GATlayer_Base):
    def __init__(self, num_in_features, num_out_features, num_heads, dropout=0.1, concat=True,activation=nn.ELU(), add_skip_connection=True, bias=True, log_attention_weights=False):
        ...
        self.bias = bias
        self.log_attention_weights = log_attention_weights
    def forward(self,nodes_features,degree_matrix):
        nodes = nodes_features
        connectivity_mask = torch.where(degree_matrix>0,0.,-1e9)
        ...
        all_attention_coefficients = torch.softmax(all_scores + connectivity_mask)
